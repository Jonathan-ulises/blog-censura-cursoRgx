...
const callModal = (title) => Swal.fire({
...
const getCalamitiesRegex = (array) => {
    ...
    const calamitiesListRgxr = new RegExp(`(${calamitiesList})`,"gi")
    return calamitiesListRgxr
}

const checkForCalamities = fn => rgx => title => {
    if (rgx.test(title.firstElementChild.textContent) && rgx.test(title.nextElementSibling.textContent)) {
        fn
    } else {
        goToPage(title)
    }
...
const blurText = (HTMLElement, checkWordsRgx) => {
    ...
    let newString = HTMLElement.replace(checkWordsRgx, `<span class="blur">${matches}</span>`)
    return newString
}
...
window.onload = () => {
    blogTitles.forEach(title => {
        title.firstElementChild.innerHTML = blurText(title.firstElementChild.textContent, getCalamitiesRegex(calamities))
    })
}